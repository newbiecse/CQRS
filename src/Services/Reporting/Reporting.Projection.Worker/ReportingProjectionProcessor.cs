...
namespace Reporting.Projection.Worker;

public sealed class ReportingProjectionProcessor(
    IServiceScopeFactory scopeFactory,
    IOptions<AzureServiceBusOptions> options,
    ILogger<ReportingProjectionProcessor> logger)
    : ServiceBusConsumerBackgroundService(options, logger)
{
    protected override string SubscriptionName => ServiceBusSubscriptions.ReportingProjection;
    ...
    protected override async Task ProcessMessageAsync(string eventType, string payload, CancellationToken cancellationToken)
    {
        ...
        await scope.ServiceProvider.GetRequiredService<ReportingProjectionHandler>()
