...
public sealed class SqlOrderReadRepository(OrderReadDbContext dbContext) : IOrderReadRepository
{
    public async Task UpsertAsync(OrderReadModel order, CancellationToken cancellationToken = default)
    {
        var entity = await dbContext.Orders.Include(o => o.Lines)
    ...
    {
        var entity = await dbContext.Orders.AsNoTracking().Include(o => o.Lines)
    ...
    {
        var entities = await dbContext.Orders.AsNoTracking().Include(o => o.Lines).ToListAsync(cancellationToken);
        return entities.Select(ToReadModel).ToList();
    }

    private static OrderReadModel ToReadModel(OrderReadEntity entity) => new()
    {
        Id = entity.Id,
        ...
        Lines = entity.Lines.Select(l => new OrderLineDto(l.ProductId, l.ProductName, l.UnitPrice, l.Quantity)).ToList()
    };
}
